#include "heart_beat/server.h"

Server::Server(int port) { 
    bzero(&server_addr, sizeof(server_addr)); 
    server_addr.sin_family = AF_INET; 
    server_addr.sin_addr.s_addr = htons(INADDR_ANY); 
    server_addr.sin_port = htons(port); // create socket to listen 
...
void Server::Bind(){ 
    if(-1 == (bind(listen_fd, (struct sockaddr*)&server_addr, sizeof(server_addr)))) { 
        cout << "Server Bind Failed!"; exit(1); 
    } 
    ...
    for(int fd=0; fd<=max_fd; ++fd){ 
        if(FD_ISSET(fd, &working_set)) {
            ...
            if(close_conn) // 当前这个连接有问题，关闭它
            {
                ...
                { 
                    while(FD_ISSET(max_fd, &master_set) == false) --max_fd; 
                } 
            } 
        } 
    } 
...
void Server::Run() { 
    ...
    while(1) { 
        ...
        if(nums < 0) { cout << "select() error!"; exit(1); 
    } 
    if(nums == 0) {
    ...
    while(1) { 
        ...
        for( ; it!=s->mmap.end(); ) { 
            ...
            { 
                ...
                {
                     while(FD_ISSET(s->max_fd, &s->master_set) == false) s->max_fd--; 
                }
